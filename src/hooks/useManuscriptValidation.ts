 ...
 */
export function useManuscriptValidation(): UseManuscriptValidationState & UseManuscriptValidationActions {
  ...
  const validateText = useCallback((text: string, label: string): ValidationResult => {
    ...
    if (!text || text.trim().length === 0) {
      ...
      // Length validation
      if (text.length < 100) {
        warnings.push(`${label} text is very short (${text.length} characters). Analysis quality may be limited.`);
      }

      if (text.length > 1000000) { // 1MB limit
        errors.push(`${label} text is too long (${text.length} characters). Maximum: 1,000,000 characters.`);
      }

      // Academic content validation
      const academicIndicators = [
        'abstract', 'introduction', 'methodology', 'results', 'discussion',
        'conclusion', 'references', 'hypothesis', 'analysis', 'data'
      ];
      ...
      if (!hasAcademicContent) {
        warnings.push(`${label} may not contain academic content. Analysis is optimized for research manuscripts.`);
      }
   ...
   */
  const validateReviewerRequestsText = useCallback((text: string): { isValid: boolean; warnings: string[] } => {
    const warnings: string[] = [];

    if (text.trim().length === 0) {
      return { isValid: true, warnings: ['No reviewer requests provided. Alignment analysis will be skipped.'] };
    }
    ...
    const requestPatterns = [
      /clarif/i, /explain/i, /detail/i, /expand/i, /improve/i,
      /method/i, /result/i, /discussion/i, /limitation/i
    ];
   ...
   */
  const setOriginalText = useCallback((content: string): void => {
   ...
   */
  const setRevisedText = useCallback((content: string): void => {
   ...
   */
  const setReviewerRequests = useCallback((content: string): void => {
  ...
  const overallValidation = useMemo((): ValidationResult => {
    ...
    if (originalFile.isValid && revisedFile.isValid) {
      ...
      if (originalFile.content.trim() === revisedFile.content.trim()) {
        warnings.push('Original and revised manuscripts appear identical. No changes will be detected.');
      }
      ...
      if (sizeDifference / averageSize > 0.8) {
        warnings.push('Large difference in manuscript sizes detected. Ensure both versions are complete.');
      }
