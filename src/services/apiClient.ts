...
export class APIClient {
  ...
  async analyzeDiffSegmentation(diffs: DiffItem[]): Promise<AnalysisItem[]> {
    try {
      ...
      console.warn('Diff segmentation API error:', error);
      throw new Error(
        `Failed to analyze diffs: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async analyzeReviewerAlignment(
    ...
    try {
      ...
      console.warn('Reviewer alignment API error:', error);
      throw new Error(
